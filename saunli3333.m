clear; clc; close all;
%% =========================================================
%  三个算例统一脚本：直接L1/FEM vs DE-SOE/FEM
%
%  PDE:
%    u_t + lambda * Caputo(u) - kappa * u_xx = f
%
%  算例1:
%    u(x,t) = t^3 sin(2*pi*x)
%
%  算例2:
%    u(x,t) = t^3 sin(pi*x) + t^2 sin(2*pi*x)
%
%  算例3:
%    u(x,t) = t^1.5 x (1-x)
%
...
for case_id = 1:3
    ...
    tbl_conv = run_time_convergence(alpha, lambda, kappa, L, T, M_conv, Nlist_conv, case_id);
    disp(tbl_conv);

    fprintf('\n========================================\n');
    fprintf('   2) 效率对比表：直接L1/FEM vs DE-SOE/FEM\n');
    fprintf('========================================\n');
    ...
    %% 保存表格
    writetable(tbl_conv, fullfile(outdir, sprintf('time_convergence_table_case%d.csv', case_id)));
    ...
    %% 作图
    plot_time_convergence(tbl_conv, outdir, case_id);
    plot_efficiency_compare(tbl_eff, outdir, case_id);
    plot_speedup(tbl_eff, outdir, case_id);

    % 代表性N下的核逼近误差曲线
    Nrep = Nlist_eff(end);
    tau_rep = T / Nrep;
    plot_kernel_error(alpha, tau_rep, T, epsSOE, hDE, 500, outdir, case_id);
...
% 时间收敛表（直接L1/FEM）
%% ==========================================================
function tbl = run_time_convergence(alpha, lambda, kappa, L, T, M, Nlist, case_id)
...
function [case_name, outdir] = get_case_info(case_id)
    ...
    switch case_id
        case 1
            ...
            outdir    = 'results_case1_t3sin2pix_DE_SOE';

        case 2
            case_name = '算例2: u(x,t)=t^3 sin(pi*x)+t^2 sin(2*pi*x)';
            outdir    = 'results_case2_t3sinpix_plus_t2sin2pix_DE_SOE';

        case 3
            case_name = '算例3: u(x,t)=t^{1.5} x (1-x)';
            outdir    = 'results_case3_t15_x1mx_DE_SOE';
...
% 绘图：时间收敛
%% ==========================================================
function plot_time_convergence(tbl_conv, outdir, case_id)

    f = figure('Visible','off');
    loglog(tbl_conv.tau, tbl_conv.L2Error, 'o-', 'LineWidth', 1.5, 'MarkerSize', 7);
    grid on;
    xlabel('\tau');
    ylabel('L2 Error at t = T');
    title(sprintf('Time Convergence of Direct L1/FEM (Case %d)', case_id));
    saveas(f, fullfile(outdir, sprintf('time_convergence_direct_case%d.png', case_id)));
    close(f);
end
...
% 绘图：CPU时间对比
%% ==========================================================
function plot_efficiency_compare(tbl_eff, outdir, case_id)

    f = figure('Visible','off');
    loglog(tbl_eff.N, tbl_eff.DirectCPU, 'o-', 'LineWidth', 1.5, 'MarkerSize', 7); hold on;
    loglog(tbl_eff.N, tbl_eff.SOECPU, 's-', 'LineWidth', 1.5, 'MarkerSize', 7);
    grid on;
    xlabel('N');
    ylabel('CPU Time (s)');
    legend('Direct L1/FEM', 'DE-SOE/FEM', 'Location', 'northwest');
    title(sprintf('CPU Time Comparison (Case %d)', case_id));
    saveas(f, fullfile(outdir, sprintf('cpu_compare_case%d.png', case_id)));
    close(f);
end
...
% 绘图：加速比
%% ==========================================================
function plot_speedup(tbl_eff, outdir, case_id)

    f = figure('Visible','off');
    semilogx(tbl_eff.N, tbl_eff.Speedup, 'o-', 'LineWidth', 1.5, 'MarkerSize', 7);
    grid on;
    xlabel('N');
    ylabel('Speedup = DirectCPU / SOECPU');
    title(sprintf('Speedup of DE-SOE/FEM (Case %d)', case_id));
    saveas(f, fullfile(outdir, sprintf('speedup_case%d.png', case_id)));
    close(f);
end
...
% 绘图：核函数相对误差
%% ==========================================================
function plot_kernel_error(alpha, tau, T, epsSOE, hDE, nTest, outdir, case_id)
    ...
    f = figure('Visible','off');
    loglog(ttest, relerr, 'LineWidth', 1.5);
    grid on;
    xlabel('t');
    ylabel('Relative Error');
    title(sprintf('Relative Error of DE-SOE Kernel Approximation (Case %d)', case_id));
    saveas(f, fullfile(outdir, sprintf('kernel_relative_error_case%d.png', case_id)));
    close(f);
end
